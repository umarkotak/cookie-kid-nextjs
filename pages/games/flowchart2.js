...
const FlowchartPuzzleGame = () => {
  ...
  // Touch handlers
  const handleTouchStart = (e, command) => {
    e.preventDefault(); // Prevent default touch behaviors
    const touch = e.touches[0];
    const rect = e.target.getBoundingClientRect();

    setDraggedCommand(command);
    setTouchDragElement(command);
    setTouchDragOffset({
  ...
  const handleTouchMove = (e) => {
    if (!touchDragElement || !dragElementRef.current) return;
    e.preventDefault();
    ...
    dragEl.style.left = `${touch.clientX - touchDragOffset.x}px`;
    dragEl.style.top = `${touch.clientY - touchDragOffset.y}px`;
  ...
  const handleTouchEnd = (e) => {
    ...
    if (elementBelow && sequenceRef.current && sequenceRef.current.contains(elementBelow)) {
      setCommandSequence([...commandSequence, { ...touchDragElement, uniqueId: Date.now() }]);
    }
    ...
    if (dragElementRef.current) {
      ...
      dragElementRef.current.style.left = '-9999px';
    }
  };
  ...
  // Add touch move listener to document
  useEffect(() => {
    if (touchDragElement) {
  ...
  }, [touchDragElement, touchDragOffset, commandSequence]);
      ...
      {touchDragElement && (
        <div
          ...
          style={{
            position: 'fixed',
            left: '-9999px',
            backgroundColor: touchDragElement.color,
            color: 'white',
        ...
        >
          <span style={{ fontSize: '1.5rem' }}>{touchDragElement.icon}</span>
          {touchDragElement.label}
        </div>
      )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem' }}>
                {commands.map(command => (
                  <div
                    ...
                    onTouchStart={(e) => handleTouchStart(e, command)}
                    style={{
