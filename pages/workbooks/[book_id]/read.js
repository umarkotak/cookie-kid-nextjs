      ...
      >
        <div className="relative h-full">
          {bookDetail.contents && bookDetail.contents.map((page, index) => (
            ...
            >
              {/* <img
                id={`workbook-image-${page.image_file_url}`}
              ...
              {/* <DrawingCanvas
                ref={drawingCanvas}
                imageElementID={`workbook-image-${page.image_file_url}`}
              ...
              >
                {/* <CanvasDraw
                  className="w-full border border-red-600 z-10"
                  imgSrc={page.image_file_url}
                /> */}
...
const DrawingCanvas = forwardRef(function DrawingCanvas({ isFullscreen, imageLoaded, imageUrl, tool, color, brushSize, bookID, pageID, imageElementID }, ref) {
  ...
  useEffect(() => {
    ...
    const drawingCanvas = drawingCanvasRef.current
    const imgDimension = getImageDimensionsSync(imageElementID)
    ...
    console.warn("CANVAS SIZE", drawingCanvas.width, drawingCanvas.height, bookID, pageID)
    const ctx = drawingCanvas.getContext('2d')
    const img = new Image()
    img.onload = () => {
      ctx.drawImage(img, 0, 0, drawingCanvas.width, drawingCanvas.height)
  ...
  return (
    <canvas
      ...
      className="mx-auto object-contain border border-red-500"
